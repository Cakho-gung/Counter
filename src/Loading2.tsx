    ...
    <>
      ...
      <div className="fixed top-0 left-0 w-full flex flex-col items-center z-40 pointer-events-none">
        ...
        <div
          ...
          className={`pointer-events-auto bg-[#151d2b] flex items-center justify-center overflow-clip transition-all duration-[1200ms] ease-[cubic-bezier(0.7,0,0.3,1)] shadow-md fixed left-1/2 -translate-x-1/2 z-40 px-[40px] py-[8px]
          ${loadingState === "loading" ? "w-[100vw] h-[100vh] top-0 rounded-none cursor-pointer max-w-[100vw]" : 
            (isScrolled ? "w-[100vw] h-[56px] max-w-[100vw] rounded-none top-0" : "w-[85%] max-w-[1088px] h-[56px] rounded-[0_0_99px_99px] top-[92px]")}
        `}
        >
          {loadingState !== "done" && (
            <div className={`absolute inset-0 pointer-events-none z-0 transition-opacity duration-[1500ms] ${
              loadingState === "loading" 
                ? (isLoaded ? "opacity-0 delay-[3300ms]" : "opacity-100 delay-0") 
                : "opacity-0 delay-0"
            }`}>
          ...
          <div className={`relative z-10 flex items-center gap-[24px] transition-transform duration-[1500ms] ease-[cubic-bezier(0.25,1,0.5,1)] ${loadingState === "loading" ? "scale-150" : "scale-100"}`}>
            ...
            <div className={`transition-all duration-[800ms] delay-[1500ms] ease-[cubic-bezier(0.25,1,0.5,1)] ${isLoaded ? "translate-y-0 opacity-100" : "translate-y-[20px] opacity-0"}`}>
              <div className="font-medium text-[16px] text-white whitespace-nowrap flex items-center" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                ...
                <span 
                  className={`inline-block whitespace-nowrap overflow-hidden transition-all duration-[1500ms] delay-[3300ms] ease-[cubic-bezier(0.25,1,0.5,1)] 
