...
function HeaderNavbar({ todayCount, show = true }: { todayCount: number, show?: boolean }) {
      ...
      <div className="fixed top-0 left-0 w-full flex flex-col items-center z-40 pointer-events-none">
        ...
        <div
          className={`w-full flex justify-center pt-[24px] px-[40px] pointer-events-auto transition-all duration-[800ms] ease-[cubic-bezier(0.25,1,0.5,1)]
            ${(!isLoaded || isScrolled) ? "-translate-y-full opacity-0" : "translate-y-0 opacity-100"}
          `}
        >
        ...
        `}
          style={{ gap: "24px", marginTop: (!isLoaded || isScrolled) ? "-100px" : "0px", opacity: isLoaded ? 1 : 0 }}
        >
...
export default function Loading1() {
  ...
  const [displayCount, setDisplayCount] = useState(() => {
    ...
    return Math.floor(secondsToday * 12); // MARKERS_PER_SECOND = 12
  });
  const startCountRef = useRef(displayCount);
    ...
    if (loadingState === "loading") {
      ...
      const tick = () => {
        ...
        timeoutId = setTimeout(() => {
          ...
          const newCount = Math.floor(startCountRef.current + elapsed * 12);
          setDisplayCount(newCount);

          if (containerRef.current) {
            const currentX = (newCount - startCountRef.current) * 2; // Reduced step: 2px per marker increase
            containerRef.current.style.transform = `translateX(${currentX}px)`;

            const rect = containerRef.current.getBoundingClientRect();
            // Close animation when it hits 70% of the screen width
    ...
    <div className="flex flex-col items-center relative w-full min-h-screen overflow-hidden bg-[#f6f6f6]" data-name="Demo" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {loadingState !== "done" && (
        <div
          ...
          className="fixed inset-0 z-50 bg-[#151d2b]"
          style={{
        ...
        >
          ...
          <div
            ref={containerRef}
            className="absolute left-[80px] bottom-0 flex items-center"
          ...
          >
            ...
            >
              {displayCount.toLocaleString("en-US", { minimumIntegerDigits: 6 }).replace(/,/g, " ")}
            </div>
          </div>

          {/* Canvas placed after text so it renders on top */}
          <div className="absolute inset-0 z-10 pointer-events-none">
            <LoadingPhysicsCanvas active={loadingState === "loading"} />
          </div>
        </div>
      )}

      {/* Header & Sticky Ticker */}
      <HeaderNavbar todayCount={todayCount} show={loadingState === "done"} />
